<script lang="ts">
	...
	const showing = $derived(
		sidebar.open === 'settings' || sidebar.open === 'account' ? sidebar.open : null
	);
</script>
...
<div class="flex h-dvh bg-background">
	...
	{#if showing}
		<aside
			class="flex w-80 shrink-0 flex-col border-r bg-rail"
			aria-label={showing === 'settings' ? 'Settings' : 'Account'}
		>
			<header class="flex h-11 shrink-0 items-center justify-between gap-2 border-b px-3">
				<h1 class="truncate text-[0.8125rem] font-medium">
					{showing === 'settings' ? 'Settings' : 'Account'}
				</h1>
				<button
					type="button"
